...
If accepted, Sprint 006 generation will use one digest-qualified Ollama model
artifact and one versioned policy. The approved identity is
`qwen3:8b@sha256:500a1f067a9f782620b40bee6f7b0c89e17ae61f686b92c24933e4ca4b2b8b41`.
Every generation operation must verify that identity immediately before and
after completion, reject drift, and use the exact defaults in this ADR. There
is no fallback model, mutable compatibility identity, automatic retry,
thinking, or tool use.
...
Sprint 006 therefore needs an explicit persisted-compatible identity, a
fail-closed drift policy, and an honest residual-race statement.
...
This decision defines the model identity, digest normalization, per-operation
identity checks, residual race handling, request defaults, capacity controls,
timeouts, failure behavior, and validation evidence for Sprint 006.
...
- The exact artifact must be an explicit compatibility contract.
- Mutable tags must never authorize a different digest silently.
- Identity drift must fail before evidence is presented as grounded.
- Request behavior must be deterministic and reviewable.
...
The compatibility identity is the tuple of provider, exact tag, canonical
SHA-256 digest, family, parameter size, quantization, operation, thinking mode,
and tool policy listed above. Mutable aliases such as `:latest` are prohibited.
...
1. resolve the exact `qwen3:8b` inventory entry;
2. canonicalize and verify all compatibility identity fields;
3. build the provider request only after verification succeeds;
4. invoke completion once;
5. resolve and verify the inventory entry again before accepting output;
6. discard output and return `generation_contract_error` if the post-check
   differs or cannot be completed.

Readiness checks are diagnostic only and do not authorize later generation.
Identity verification must not use a permanent success cache.

### Residual race and operational control

The pre-check, generation call, and post-check are separate HTTP operations.
They cannot prove that no tag mutation occurred and was reversed between
checks. Sprint 006 therefore does not claim atomic model-identity binding.
...
traffic. Post-check drift detection remains mandatory defense in depth. If
exclusive mutation control cannot be demonstrated in the validation
environment, implementation approval remains blocked.
...
contract or body limit fails as `generation_invalid_response`.
...
- provider timeout: `generation_timeout`;
- invalid structured output: `generation_invalid_response`;
...
Queue overflow or expiry returns `interaction_overloaded`. The whole-request
deadline bounds retrieval, assembly, generation, identity checks, and response
validation. Client cancellation stops downstream work where possible and
prevents persistence or a partial grounded response.
...
### Claim pre/post checks are atomic

Rejected because the provider does not bind inventory identity and completion
in one transaction.
...
- Generation behavior is tied to one explicit artifact and policy.
- Model drift fails visibly rather than degrading silently.
- Determinism and resource limits are mechanically testable.
- The design makes no unsupported atomicity claim.
...
This generation identity is independent of the accepted Sprint 005 embedding
identity `nomic-embed-text:v1.5`. No vector, memory payload, or Qdrant migration
is required. A future model or digest requires a new reviewed compatibility
decision and policy validation.

## Validation requirements

- exact Ollama inventory-shape digest canonicalization tests;
