...
- ADR 0014 is Accepted, and its bounded, metadata-only Knowledge Registry
  ...
  Knowledge Vault remains **Named**. The separate terminally closed Phase 2
  package implements only disconnected synthetic document-admission and
  inspection contracts over generated bytes and does not write the registry.
  No real source or derived content, real document ingestion, production
  parsing, organizational-information authorization, durable store, Qdrant or
  memory integration, retrieval, runtime consumer, or deployment exists.
